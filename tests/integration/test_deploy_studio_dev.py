"""Deploy the Stasis protocol to a live network and register the mock target.
...
# Registration parameters for the mock target. Threshold sits below the
# simulated breach so the connectivity incident actually trips the guardian.
THRESHOLD_BPS = 500
BOUNTY_AMOUNT = 0
COOLDOWN_SECONDS = 0
...
def test_deploy_and_register():
    ...
    vault = get_contract_factory(contract_file_path="mock_vault.py").deploy(
        args=[],
        fees=live_fees(),
        wait_until="finalized",
        wait_interval=WAIT_INTERVAL_MS,
    ...
    print(f"mock_vault       : {vault.address}")
        ...
        json.dump(
            {
                ...
                "mockVaultRegistered": bool(registered),
            },
            fh,
            indent=2,
        )
